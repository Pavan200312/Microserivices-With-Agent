-- Migration script to adapt existing database schema
-- Run this after connecting to your existing database

-- Check if tables exist and create if they don't
DO $$ 
BEGIN
    -- Create commits table if it doesn't exist
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'commits') THEN
    ...
    -- Create tracking_sessions table if it doesn't exist
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'tracking_sessions') THEN
    ...
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'ai_analysis') THEN
